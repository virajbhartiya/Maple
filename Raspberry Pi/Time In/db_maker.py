...
def dbMaker(path):
    ...
    df = xl = pd.ExcelFile("student_db/people_db.xlsx")
    df = xl.parse("Sheet1")
    df = df.sort_values(by='Name')

    writer = pd.ExcelWriter('student_db/output.xlsx')
...
def createSheet(names, roll):
    ...
    seperator.join(roll)
    update = requests.get(
        f'https://script.google.com/macros/s/AKfycbyOMUi0Vh-_ZxNJZhqtWJl3_dz-mS5O213GMFyeb6l6fmw4b_4MMY7WLA/exec?key=create&names={names}&roll={roll}').text
